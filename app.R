...
ui <- navbarPage(
  ...
  tabPanel("Multiple Linear Regresion",
           fluidPage(
             titlePanel(title = div("Multiple Linear Regresion", style = "color: #FF4040; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
             mainPanel(
  ...
  tabPanel("Correlation Plot",
           fluidPage(
             titlePanel(title = div("Correlation Plot", style = "color: #FF4040; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
             mainPanel(
               style = "margin: 0 auto; text-align: center; margin-left: 300px;",
  ...
  tabPanel("Variable Relationship",
           fluidPage(
             titlePanel(title = div("Variable Relationship Analysis Dashboard", style = "color: #FF4040; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
             sidebarLayout(
               sidebarPanel(
  ...
  tabPanel("Prediction Dashboard",
           fluidPage(
             titlePanel(title = div("Prediction Dashboard Monthly Sales", style = "color: #FF4040; font-size: 40px; font-weight: bold; text-align: center; height: 120px")),
             sidebarLayout(
               sidebarPanel(
                 numericInput("x1_input", "x1 Value:", value = 0),
                 numericInput("x2_input", "x2 Value:", value = 8000),
                 numericInput("x3_input", "x3 Value:", value = 0),
                 numericInput("x4_input", "x4 Value:", value = 0),
  ...
  observeEvent(input$plot, {
    ...
    output$scatter_boxplot <- renderPlot({
      ...
      scatter_plot <- ggplot(data, aes_string(x = variable1, y = "y")) +
        ...
        geom_smooth(method = "lm", se = FALSE, color = "#8DEEEE") +
        labs(x = variable1, y = "y", title = paste("Scatter Plot of", variable1, "vs. y")) +
        theme_minimal() +
        theme(plot.title = element_text(size = 16, face = "bold", hjust = 0.5, color = "#C1CDC1"),
